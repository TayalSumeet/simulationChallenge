...
ax1.set_title(f'Single Simulation: Account Balance Over Time (Age {starting_age} → {final_age})', 
...
ax2.set_title('Coin Flip Sequence (Green = Heads ↑, Red = Tails ↓)', 
...
# Expected log return = p × ln(1 + f × gain) + q × ln(1 - f × loss)
...
# - Win: balance becomes (1-f) + f×1.5 = 1 + 0.5f
# - Lose: balance becomes (1-f) + f×0.6 = 1 - 0.4f
# Kelly maximizes: p×ln(1 + 0.5f) + q×ln(1 - 0.4f)
# Taking derivative and setting to zero:
# p×0.5/(1+0.5f) = q×0.4/(1-0.4f)
...
print(f"\nInterpretation:")
if kelly_fraction > 0:
    print(f"  • The Kelly Criterion suggests betting {kelly_fraction:.1%} of your balance")
    print(f"  • This maximizes long-term growth rate")
    print(f"  • Betting more than {kelly_fraction:.1%} increases risk without increasing growth")
    print(f"  • Betting less than {kelly_fraction:.1%} is safer but suboptimal for growth")
else:
    print(f"  • The Kelly fraction is negative or zero")
    print(f"  • This suggests the game has negative expected value")
    print(f"  • The optimal strategy would be to not bet at all")

print(f"\nComparison to Our Strategies:")
print(f"  • Original strategy: bets 100% of balance (full risk)")
print(f"  • Modified strategy: bets 50% of balance (partial risk)")
print(f"  • Kelly optimal: bets {kelly_fraction:.1%} of balance")
print(f"\n  • The modified strategy ({50:.1%}) is {'closer' if abs(50 - kelly_fraction*100) < abs(100 - kelly_fraction*100) else 'farther'} to the Kelly optimal")
...
- Heads (50% chance): gain 50% → win multiplier = 1.5
- Tails (50% chance): lose 40% → loss multiplier = 0.6
